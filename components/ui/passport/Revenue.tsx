...
export default function Revenue({ slug }: DuneDataProps) {
  ...
  if (isLoading)
    return (
      <div className="h-8 w-24 animate-pulse mt-0.5 rounded bg-gray-200 dark:bg-gray-800"></div>
    );
  ...
  return (
    <>
      <dd className="text-lg font-semibold text-gray-900 dark:text-gray-50 mt-1.5 flex items-center gap-2">
        {new Intl.NumberFormat("en-US").format(data[0]["ETH Revenue"])} ETH
      </dd>
    </>
  );
}
